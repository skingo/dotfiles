function vim8 --description 'start vim with a server name to avoid server name duplication'
	...
	set used_names (command vim8 --serverlist)
	...
	if [ -z $server_name ]
        ...
        command vim8 --servername $server_name -c "echohl MoreMsg | echomsg \"Server name: $server_name\" | echohl None" $argv
	end
end
